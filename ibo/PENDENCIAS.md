...
- [x] **Mesa de Trabalho por capítulo** — ambiente funcional em `/mesa-trabalho`, integrado à árvore documental real. Reúne navegação de capítulos, leitura contínua, seleção contextual, comparação com o vigente, consulta aos subsídios, status e inclusão de subordinados. A reorganização ocorre na própria Mesa, com seleção de destino/posição e prévia completa da renumeração; redação e colaboração abrem diretamente na aba correspondente e oferecem retorno ao mesmo contexto da Mesa. Sem migração de banco.
