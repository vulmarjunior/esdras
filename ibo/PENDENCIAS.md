...
- [x] **Mesa de Trabalho por capítulo** — primeira versão funcional em `/mesa-trabalho`, integrada à árvore documental real. Reúne navegação de capítulos, leitura contínua, seleção contextual, comparação com o vigente, consulta aos subsídios, status, inclusão de subordinados e atalhos para redação, reorganização, discussão e deliberação. Sem migração de banco.
