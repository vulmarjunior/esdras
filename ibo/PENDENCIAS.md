...
- [x] **Apreciação em 3 estados e exportação da nova minuta (26/09/2026)** — dispositivos com **Pendente de análise / Em análise / Apreciado pela comissão**, com rubrica marginal (forma + cor + rótulo acessível), controle segmentado na barra, resumo e pontos no sumário, selo na visualização e contadores na home; editar o texto de um apreciado devolve para "em análise" (o campo `approved` legado é normalizado no `validate`, **sem migração de banco**). Exportação em **HTML autocontido**, **Markdown** e **impressão/PDF** em folha limpa (`/mesa-trabalho/imprimir`; downloads por `/api/export/nova-mesa`), com legenda, rubricas, cabeçalho institucional e opções de sumário/somente-apreciados. Testes Vitest 220; lint sem erros (corrigidos os 3 erros antigos do editor); build verde.
- [ ] **Manual do usuário para a nova Mesa** — `lib/manual.ts` ainda descreve a Mesa clássica por dispositivo; atualizar a seção para a minuta independente (apreciação em 3 estados, exportação/impressão e visualização somente leitura).
