...
export async function createMeeting(data: {
  ...
  const members = all<{ id: number }>("SELECT id FROM users WHERE role IN ('coordenador','membro')");
  for (const m of members) {
    run("INSERT INTO meeting_members (meeting_id, user_id) VALUES (?, ?)", [Number(res.lastInsertRowid), m.id]);
  }
  audit(user.id, user.name, "Criou reunião #" + data.numero, "meeting", String(res.lastInsertRowid));
...
export async function startMeeting(meetingId: number): Promise<ActionState> {
  const user = await requireRole("coordenador", "admin");
  run("UPDATE meetings SET status = 'em_andamento', started_at = ? WHERE id = ?", [now(), meetingId]);
  run("INSERT INTO meeting_events (meeting_id, user_id, hora, tipo, descricao) VALUES (?, ?, ?, 'inicio', 'Reunião iniciada')", [
    meetingId,
    user.id,
    new Date().toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit" }),
  ]);
  audit(user.id, user.name, "Iniciou reunião", "meeting", String(meetingId));
...
export async function endMeeting(meetingId: number): Promise<ActionState> {
  const user = await requireRole("coordenador", "admin");
  run("UPDATE meetings SET status = 'encerrada', ended_at = ? WHERE id = ?", [now(), meetingId]);
  run("INSERT INTO meeting_events (meeting_id, user_id, hora, tipo, descricao) VALUES (?, ?, ?, 'encerramento', 'Reunião encerrada')", [
    meetingId,
    user.id,
    new Date().toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit" }),
  ]);
  audit(user.id, user.name, "Encerrou reunião", "meeting", String(meetingId));
  revalidatePath(`/reunioes/${meetingId}`);
  return { ok: true };
}
...
export async function setPresence(meetingId: number, userId: number, presente: boolean): Promise<ActionState> {
  ...
  run("UPDATE meeting_members SET presente = ? WHERE meeting_id = ? AND user_id = ?", [presente ? 1 : 0, meetingId, userId]);
  revalidatePath(`/reunioes/${meetingId}`);
  return { ok: true };
}
...
export async function addManualEvent(meetingId: number, descricao: string): Promise<ActionState> {
  ...
  run("INSERT INTO meeting_events (meeting_id, user_id, hora, tipo, descricao) VALUES (?, ?, ?, 'registro', ?)", [
    meetingId,
    user.id,
    new Date().toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit" }),
    descricao,
  ]);
  audit(user.id, user.name, "Registrou evento em reunião", "meeting", String(meetingId), descricao);
  revalidatePath(`/reunioes/${meetingId}`);
  return { ok: true };
}
...
export async function addDecision(
  ...
  const meeting = get<{ numero: number; data: string }>("SELECT numero, data FROM meetings WHERE id = ?", [meetingId]);
  if (!meeting) return { error: "Reunião não encontrada." };
  const code = `DEC-${meeting.data.slice(0, 10)}-${String(meeting.numero).padStart(3, "0")}`;
  const n = get<{ c: number }>("SELECT COUNT(*) c FROM meeting_decisions WHERE meeting_id = ?", [meetingId])?.c ?? 0;
  const fullCode = `${code}-${String(n + 1).padStart(3, "0")}`;
  run("INSERT INTO meeting_decisions (code, meeting_id, provision_id, tipo, texto, user_id) VALUES (?, ?, ?, ?, ?, ?)", [
    fullCode,
    meetingId,
    provisionId,
    tipo,
  ...
  run("INSERT INTO meeting_events (meeting_id, user_id, hora, tipo, descricao) VALUES (?, ?, ?, 'deliberacao', ?)", [
    meetingId,
    user.id,
    new Date().toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit" }),
    `${fullCode} — ${texto.slice(0, 140)}`,
  ]);
  audit(user.id, user.name, "Registrou deliberação " + fullCode, "meeting", String(meetingId));
  revalidatePath(`/reunioes/${meetingId}`);
...
export async function updateMeeting(
  ...
  const user = await requireRole("coordenador", "admin");
  const exists = get("SELECT id FROM meetings WHERE id = ?", [meetingId]);
  ...
  audit(user.id, user.name, "Atualizou reunião", "meeting", String(meetingId));
  revalidatePath(`/reunioes/${meetingId}`);
  revalidatePath("/reunioes");
...
export async function deleteMeeting(meetingId: number): Promise<ActionState> {
  const user = await requireRole("coordenador", "admin");
  const meeting = get<{ numero: number }>("SELECT numero FROM meetings WHERE id = ?", [meetingId]);
  if (!meeting) return { error: "Reunião não encontrada." };
  const ataAprovada = get<{ c: number }>(
  ...
  transaction(() => {
    run("DELETE FROM meeting_decisions WHERE meeting_id = ?", [meetingId]);
    run("DELETE FROM meeting_events WHERE meeting_id = ?", [meetingId]);
    run("DELETE FROM meeting_members WHERE meeting_id = ?", [meetingId]);
    run("DELETE FROM minutes WHERE meeting_id = ?", [meetingId]);
    run("DELETE FROM meetings WHERE id = ?", [meetingId]);
    audit(user.id, user.name, "Excluiu reunião nº " + meeting.numero, "meeting", String(meetingId));
  });
  revalidatePath("/reunioes");
...
export async function generateMinutes(meetingId: number): Promise<ActionState> {
  ...
  const total = get<{ c: number }>("SELECT COUNT(*) c FROM meeting_members WHERE meeting_id = ?", [meetingId])?.c ?? 0;
  const eventos = all<{ hora: string; descricao: string }>(
    "SELECT hora, descricao FROM meeting_events WHERE meeting_id = ? ORDER BY id",
    [meetingId]
  );
  const decisoes = all<{ code: string; texto: string; provision_id: string | null }>(
  ...
  run(`INSERT INTO minutes (meeting_id, status, conteudo) VALUES (?, 'rascunho', ?)
  ...
  audit(user.id, user.name, "Gerou minuta de ata", "meeting", String(meetingId));
  revalidatePath(`/reunioes/${meetingId}`);
...
export async function setMinutesStatus(minutesId: number, status: string): Promise<ActionState> {
  const user = await requireRole("coordenador", "admin");
  const m = get<{ meeting_id: number }>("SELECT meeting_id FROM minutes WHERE id = ?", [minutesId]);
  if (!m) return { error: "Minuta não encontrada." };
  run("UPDATE minutes SET status = ?, updated_at = datetime('now') WHERE id = ?", [status, minutesId]);
  audit(user.id, user.name, "Ata em status " + status, "minutes", String(minutesId));
...
export async function saveMinutes(meetingId: number, content: string): Promise<ActionState> {
  ...
  run(`INSERT INTO minutes (meeting_id, status, conteudo) VALUES (?, 'rascunho', ?)
       ON CONFLICT(meeting_id) DO UPDATE SET conteudo = excluded.conteudo, status = 'rascunho', updated_at = datetime('now')`,
    [meetingId, content]);
  audit(user.id, user.name, "Atualizou minuta de ata", "meeting", String(meetingId));
  revalidatePath(`/reunioes/${meetingId}`);
  return { ok: true };
}
...
export async function reviewMinutes(minutesId: number, opinion: string, content: string): Promise<ActionState> {
  ...
  const m = get<{ meeting_id: number }>("SELECT meeting_id FROM minutes WHERE id = ?", [minutesId]);
  if (!m) return { error: "Minuta não encontrada." };
  run("INSERT INTO minutes_reviews (minutes_id, user_id, opinion, content) VALUES (?, ?, ?, ?)", [
    minutesId,
    user.id,
    opinion,
    content || "",
