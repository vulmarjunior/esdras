...
export async function GET(req: NextRequest) {
  ...
  if (type === "consolidado") {
    const rows = all<{ id: string; type: string; numero: string | null; titulo: string | null; redacao: string }>(`
    ...
    for (const r of rows) {
      ...
      if (r.type === "capitulo") {
        lines.push("", label.toUpperCase() + (r.titulo ? ` — ${r.titulo}` : ""), "-".repeat(40), "");
      } else if (r.type === "artigo") {
        lines.push(`${label} — ${r.redacao}`);
      } else if (r.type === "paragrafo") {
        lines.push(`    ${label.toLowerCase()}: ${r.redacao}`);
      } else if (r.type === "inciso") {
        lines.push(`        ${r.numero}) ${r.redacao}`);
      } else {
        lines.push(`${label}: ${r.redacao}`);
      }
    }
  ...
  if (type === "comparativo") {
    const rows = all<{ id: string; numero: string | null; vigente: string; nova: string; tipo: string }>(`
    ...
    for (const r of rows) {
      ...
      lines.push(`  VIGENTE: ${r.vigente || "(não existe)"}`);
      lines.push(`  NOVA   : ${r.nova || "(sem alteração)"}`);
      lines.push("");
    }
  ...
  if (type === "reforma") {
    const rows = all<{ numero: string | null; alteracao: string; justificativa: string }>(`
    ...
    for (const r of rows) {
      ...
      if (r.justificativa) lines.push(`  Justificativa: ${r.justificativa}`);
      lines.push("");
    }
  ...
  if (type === "fundamentacao") {
    const rows = all<{ id: string; numero: string | null }>("SELECT id, numero FROM provisions WHERE type='artigo' ORDER BY ordem_pai");
    const lines: string[] = ["RELATÓRIO DE FUNDAMENTAÇÃO", "=".repeat(60), ""];
    for (const r of rows) {
      lines.push(`Art. ${r.numero}`);
      for (const t of ["biblica", "doutrinaria", "juridica", "pastoral"] as const) {
        const refs = all<{ texto: string }>("SELECT texto FROM references_tb WHERE provision_id=? AND tipo=?", [r.id, t]);
        if (refs.length) {
  ...
  if (type === "historico") {
    ...
    const meetings = all<{ id: number; numero: number; data: string }>("SELECT id, numero, data FROM meetings ORDER BY data");
    lines.push("REUNIÕES:");
    for (const m of meetings) {
      const dec = all<{ code: string }>("SELECT code FROM meeting_decisions WHERE meeting_id=?", [m.id]);
      lines.push(`  Reunião ${m.numero} (${m.data}) — ${dec.length} deliberações`);
    }
    const decisions = all<{ code: string; texto: string }>("SELECT code, texto FROM meeting_decisions ORDER BY id");
    if (decisions.length) {
      lines.push("", "DELIBERAÇÕES:");
      for (const d of decisions) lines.push(`  ${d.code} — ${d.texto}`);
    }
    const aprovados = all<{ numero: string | null }>("SELECT numero FROM provisions WHERE status='aprovado' AND type='artigo' ORDER BY ordem");
    if (aprovados.length) {
      lines.push("", `ARTIGOS APROVADOS (${aprovados.length}): ${aprovados.map((a) => a.numero).join(", ")}`);
    }
    const pendings = all<{ categoria: string; descricao: string; status: string }>("SELECT categoria, descricao, status FROM pending_issues ORDER BY id");
    if (pendings.length) {
  ...
  if (type === "atas") {
    const rows = all<{ meeting_id: number; numero: number; data: string; conteudo: string | null; status: string }>(`
